...
Ext.ux.grid.BufferView = Ext.extend(Ext.grid.GridView, {rowHeight: 19, borderHeight: 2, scrollDelay: 100, cacheSize: 20, cleanDelay: 500, initTemplates: function () {
    ...
    var b = this.templates, f = b.cell, h = b.row, x = b.rowBody, n = s - 1;
    ...
    if (this.getVisibleRowCount() > 0) {
        var f = this.grid, b = f.colModel, h = f.store;
...
Ext.ux.grid.CheckColumn.prototype = {init: function (a) {
    this.grid = a;
...
Ext.ux.grid.GridFilters = Ext.extend(Ext.util.Observable, {autoReload: true, filterCls: "ux-filtered-column", local: false, menuFilterText: "Filters", paramPrefix: "filter", showMenu: true, stateId: undefined, updateBuffer: 500, constructor: function (a) {
    ...
    var b = this.grid.getView(), e, c, a, d;
    if (b.mainHd) {
        e = b.mainHd.select("td").removeClass(this.filterCls);
        for (c = 0, a = b.cm.config.length; c < a; c++) {
            d = this.getFilter(b.cm.config[c].dataIndex);
            if (d && d.active) {
                e.item(c).addClass(this.filterCls)
            }
        }
    }
}, reload: function () {
...
Ext.ux.grid.GroupSummary = Ext.extend(Ext.util.Observable, {constructor: function (a) {
    ...
    var a = this.view.getGroups(), k, h, e = g.length;
...
Ext.ux.grid.LockingGridView = Ext.extend(Ext.grid.GridView, {lockText: "Lock", unlockText: "Unlock", rowBorderWidth: 1, lockedBorderWidth: 1, syncHeights: false, initTemplates: function () {
...
}, updateSortIcon: function (c, b) {
    var g = this.sortClasses, d = this.lockedHd.select("td").removeClass(g), f = this.mainHd.select("td").removeClass(g), e = this.cm.getLockedCount(), a = g[b == "DESC" ? 1 : 0];
    if (c < e) {
        d.item(c).addClass(a)
    } else {
        f.item(c - e).addClass(a)
    }
    ...
    for (var c = 0, d = m.length; c < d; ++c) {
        ...
        j.rowIndex = c;
        if (!g) {
            l.className = l.className.replace(this.rowClsRe, " ");
            j.className = j.className.replace(this.rowClsRe, " ");
            if ((idx + 1) % 2 === 0) {
                l.className += " x-grid3-row-alt";
                j.className += " x-grid3-row-alt"
            }
        }
        if (this.syncHeights) {
            var b = Ext.get(l), a = Ext.get(j), h = b.getHeight(), f = a.getHeight();
            if (h > f) {
                a.setHeight(h)
            } else {
                if (f > h) {
    ...
    if (k === 0) {
        Ext.fly(m[0]).addClass(this.firstRowCls);
    ...
    Ext.ux.grid.LockingGridView.superclass.refreshRow.call(this, a);
...
}, moveColumn: function (b, a) {
    if (b < a && this.isLocked(b) && !this.isLocked(a)) {
        this.setLocked(b, false, true)
    } else {
        if (b > a && !this.isLocked(b) && this.isLocked(a)) {
            this.setLocked(b, true, true)
        }
    }
...
Ext.extend(Ext.ux.form.MultiSelect.DropZone, Ext.dd.DropZone, {getTargetFromEvent: function (b) {
...
}, onNodeDrop: function (b, k, j, g) {
    if (this.ms.fireEvent("drop", this, b, k, j, g) === false) {
        return false
    }
    ...
    var d = false;
    if (g.sourceView == this.view) {
        if (l == "below") {
            if (g.viewNodes[0] == b) {
                g.viewNodes.shift()
            }
        } else {
            if (g.viewNodes[g.viewNodes.length - 1] == b) {
                g.viewNodes.pop()
            }
        }
        if (!g.viewNodes.length) {
            return false
        }
        if (c > this.view.store.indexOf(g.records[0])) {
            d = "down";
            c--
        }
    }
    for (var h = 0; h < g.records.length; h++) {
        var a = g.records[h];
        if (g.sourceView) {
            g.sourceView.store.remove(a)
        }
        this.view.store.insert(d == "down" ? c : c++, a);
        var f = this.view.store.sortInfo;
        if (f) {
            this.view.store.sort(f.field, f.direction)
        }
    }
    return true
...
Ext.extend(Ext.ux.Portal.DropZone, Ext.dd.DropTarget, {ddScrollConfig: {vthresh: 50, hthresh: -1, animate: true, increment: 200}, createEvent: function (a, f, d, b, h, g) {
    ...
    if (this.portal.fireEvent("validatedrop", a) !== false && this.portal.fireEvent("beforedrop", a) !== false) {
        l.proxy.getProxy().remove();
        l.panel.el.dom.parentNode.removeChild(l.panel.el.dom);
        if (k !== false) {
            if (j == l.panel.ownerCt && (j.items.items.indexOf(l.panel) <= k)) {
                k++
            }
            j.insert(k, l.panel)
        } else {
...
Ext.ns("Ext.ux.grid");
Ext.ux.grid.RowExpander = Ext.extend(Ext.util.Observable, {expandOnEnter: true, expandOnDblClick: true, header: "", width: 20, sortable: false, fixed: true, menuDisabled: true, dataIndex: "", id: "expander", lazyRender: true, enableCaching: true, constructor: function (a) {
    Ext.apply(this, a);
...
Ext.ns("Ext.ux.layout");
Ext.ux.layout.RowLayout = Ext.extend(Ext.layout.ContainerLayout, {monitorResize: true, isValidParent: function (b, a) {
    return b.getEl().dom.parentNode == this.innerCt.dom
}, onLayout: function (f, j) {
    var b = f.items.items, g = b.length, a, c;
    if (!this.innerCt) {
        j.addClass("ux-row-layout-ct");
    ...
    for (c = 0; c < g; c++) {
        a = b[c];
        if (a.rowHeight) {
            a.setSize({height: Math.floor(a.rowHeight * e) - a.getEl().getMargins("tb")})
        }
    }
}});
...
Ext.ux.SliderTip = Ext.extend(Ext.Tip, {minWidth: 10, offsets: [0, -10], init: function (a) {
...
Ext.ux.SlidingPager = Ext.extend(Object, {init: function (a) {
    Ext.each(a.items.getRange(2, 6), function (d) {
        d.hide()
    });
    var b = new Ext.Slider({width: 114, minValue: 1, maxValue: 1, plugins: new Ext.ux.SliderTip({getText: function (c) {
        return String.format("Page <b>{0}</b> of <b>{1}</b>", c.value, c.maxValue)
    }}), listeners: {changecomplete: function (d, c) {
        a.changePage(c)
    }}});
    a.insert(5, b);
    a.on({change: function (c, d) {
        b.maxValue = d.pages;
...
Ext.ux.TabCloseMenu = function () {
    ...
    function d(h, g, j) {
        if (!c) {
            c = new Ext.menu.Menu({items: [
                {id: a.id + "-close", text: "Close Tab", handler: function () {
                    a.remove(b)
                }},
                {id: a.id + "-close-others", text: "Close Other Tabs", handler: function () {
...
(function () {
    Ext.override(Ext.list.Column, {init: function () {
        ...
        if (typeof this.sortType == "string") {
            this.sortType = a[this.sortType]
        }
        if (!this.sortType) {
            switch (this.type) {
                case"string":
                    this.sortType = a.asUCString;
                    break;
                case"date":
                    this.sortType = a.asDate;
                    break;
                default:
                    this.sortType = a.none
            }
        }
    }});
...
Ext.tree.ColumnResizer = Ext.extend(Ext.util.Observable, {minWidth: 14, constructor: function (a) {
    ...
    var b = this.proxy.getWidth(), a = this.tree;
    this.proxy.remove();
    delete this.dragHd;
...
Ext.ux.tree.TreeGridSorter = Ext.extend(Ext.tree.TreeSorter, {sortClasses: ["sort-asc", "sort-desc"], sortAscText: "Sort Ascending", sortDescText: "Sort Descending", constructor: function (a, b) {
    ...
    this.defaultSortFn = function (k, j) {
        var f = me.dir && me.dir.toLowerCase() == "desc";
        ...
        if (g) {
            if (k.attributes[d] && !j.attributes[d]) {
                return 1
            }
            if (!k.attributes[d] && j.attributes[d]) {
                return -1
